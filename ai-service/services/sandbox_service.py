...
class SandboxService():
    ...
    class SandboxListTool(BaseTool):
        ...
        def _run(self, sandbox_id: str, path: str) -> str:
            try:
                files: List[WriteInfo] = self.sandbox_service.list_files(sandbox_id=sandbox_id, path=path)
    ...
    class SandboxReadTool(BaseTool):
        ...
        description: str = "Read a single file in the sandbox. To access the sandbox, the first paramater must be the sandbox_id and the second must be the path of the file."
        sandbox_service: 'SandboxService' = Field(exclude=True)
        args_schema: Type[BaseModel] = ReadToolInput 

        def _run(self, sandbox_id:str, path:str) -> str:
            try:
                file_content:str = self.sandbox_service.read_file(sandbox_id=sandbox_id, path=path)
    ...
    class SandboxWriteTool(BaseTool):
        ...
        def _run(self, sandbox_id: str, write_data: List[WriteEntry]) -> str:
            try:
                ...
                written_files = []
                for file in result:
                    written_files.append(f"Successfully wrote {file.type}: {file.name} to: {file.path}")
    ...
    class SandboxCommandTool(BaseTool):
        ...
        def _run(self, sandbox_id: str, command: str) -> str:
            try:
                result: TerminalInfo = self.sandbox_service.execute_terminal_command(sandbox_id=sandbox_id, command=command)
